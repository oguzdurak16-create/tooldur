...
import { ChevronDown, ChevronRight, Globe, LayoutDashboard, LogIn, Menu, Search, X } from 'lucide-react';
    ...
    <>
      <header className={styles.header}>
        <div className={styles.inner}>
          ...
          <div className={styles.actions}>
            ...
            <button className={`${styles.iconButton} ${styles.mobileButton}`} type="button" onClick={() => setMenuOpen(true)} aria-label={nav.menu}><Menu size={19} /></button>
          </div>
        </div>
      </header>
      ...
      <aside className={`${styles.drawer} ${menuOpen ? styles.drawerOpen : ''}`} aria-hidden={!menuOpen}>
        <div className={styles.drawerTop}>
          <Link href={homeHref} className={styles.brand} aria-label="Tooldur ana sayfa"><span className={styles.mark}>T</span><span className={styles.brandText}><span className={styles.brandName}>tool<span>dur</span></span><span className={styles.brandTag}>engineering tools</span></span></Link>
          <button className={styles.iconButton} type="button" onClick={() => setMenuOpen(false)} aria-label={nav.closeMenu}><X size={19} /></button>
        </div>

        <button className={styles.drawerSearch} type="button" onClick={openSearch}><Search size={18} /><span>Araç ara</span><kbd>/</kbd></button>
        <nav className={styles.drawerNav} aria-label="Mobil menü">{drawerLinks.map((item) => <Link key={item.href} href={item.href} className={`${styles.drawerLink} ${isActive(item.href) ? styles.active : ''}`}>{item.label}<ChevronRight size={16} /></Link>)}</nav>

        <div className={styles.drawerBottom}>
          {authReady && user ? <Link href="/dashboard" className={`${styles.drawerLink} ${styles.active}`}>{nav.panel}<LayoutDashboard size={16} /></Link> : <Link href="/giris" className={`${styles.drawerLink} ${styles.active}`}>{nav.login}<LogIn size={16} /></Link>}
          <div className={styles.languageGrid}>{LANGUAGES.slice(0, 6).map((item) => <button key={item.code} type="button" onClick={() => changeLanguage(item.code)}>{item.short}</button>)}</div>
        </div>
      </aside>
