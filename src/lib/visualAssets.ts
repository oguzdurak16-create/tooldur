...
export function getToolVisual(categoryId?: string, slug = ''): SiteVisual {
  ...
  if (tool) {
    const path = `/api/tool-visual/${encodeURIComponent(tool.slug)}.svg`;
    return {
      src: path,
      og: path,
      alt: `${tool.name} için özel teknik illüstrasyon`,
      caption: `${tool.shortName} hesaplama çalışma alanı`,
    };
  }
  return getCategoryVisual(categoryId);
}
...
export function getBlogVisual(post: { title: string; category: string; keywords?: string[] }): SiteVisual {
  ...
  if (/(mil|şaft|saft|konik)/.test(text)) return blogVisuals.shaft;
  if (/(yay)/.test(text)) return blogVisuals.spring;
  if (/(tolerans|geçme|gecme|o-ring|oring|kaynak)/.test(text)) return blogVisuals.tolerance;
  if (/(pompa)/.test(text)) return blogVisuals.pump;
  if (/(basınç|basinc|reynolds|viskozite|akış|akis|boru)/.test(text)) return blogVisuals.fluid;
  if (/(termal|ısıl|isil|genleşme|genlesme)/.test(text)) return blogVisuals.thermal;
  if (/(kablo|voltaj|gerilim)/.test(text)) return blogVisuals.electricalCable;
  if (/(elektrik|ohm|led|güç faktörü|guc faktoru)/.test(text)) return blogVisuals.electricalCircuit;
  if (/(beton|donatı|donati|demir|merdiven|tuğla|tugla)/.test(text)) return blogVisuals.concrete;
  if (/(profil|çelik|celik|ağırlık|agirlik)/.test(text)) return blogVisuals.profile;
  if (/(kimya|molarite|seyreltme|çözelti|cozelti)/.test(text)) return blogVisuals.chemistry;
  if (/(enerji|güneş|gunes|karbon|çevre|cevre)/.test(text)) return blogVisuals.energy;
  if (/(tooldurcad|solidworks|cad|yazılım|yazilim|api|sla)/.test(text)) return blogVisuals.software;
  if (/(oee|takt|üretim|uretim|verimlilik|kapasite)/.test(text)) return blogVisuals.production;
  if (/(birim|çevir|cevir|uzunluk|sıcaklık|sicaklik)/.test(text)) return blogVisuals.units;
  if (/(kredi|kdv|fiyat|pazaryeri|kira|finans)/.test(text)) return blogVisuals.finance;
  if (/(bmi|vücut kitle|vucut kitle)/.test(text)) return blogVisuals.health;
  return categoryVisuals[post.category] || blogVisuals.general;
}
