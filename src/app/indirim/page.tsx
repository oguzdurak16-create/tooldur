...
export default function IndirimPage(){
  ...
  const sectionProducts=useMemo(()=>products.filter(p=>productProfile(p)===section),[products,section]);
  const familyProducts=useMemo(()=>sectionProducts.filter(isFamily).sort((a,b)=>Number(b.discount_percent||0)-Number(a.discount_percent||0)||Number(a.current_price||99999999)-Number(b.current_price||99999999)),[sectionProducts]);
  const coreProducts=useMemo(()=>sectionProducts.filter(p=>!isFamily(p)),[sectionProducts]);
  ...
  const load=async(silent=false)=>{
    ...
    const[c,s,r,st,l,...pq]=await Promise.all([
    ...
    const all=[c,s,r,st,l,...pq];const firstError=all.find((x:any)=>x?.error)?.error;if(firstError)setError(firstError.message||'Veriler alınamadı.');
    const merged=new Map<string,Product>();for(const q of pq){for(const p of(q?.data||[]))merged.set(p.id,p as Product)}
    setCategories((c.data||[])as Category[]);setSites((s.data||[])as Site[]);setRules((r.data||[])as Rule[]);setProducts(Array.from(merged.values()));setLogs((l.data||[])as ScanLog[]);if(st.data)setSettings(st.data as Settings);if(!silent)setLoading(false);
  };
  ...
  const realDrops=sectionProducts.filter(p=>Number(p.discount_percent||0)>0).length;const lastScan=logs[0];const lastScanText=lastScan?new Date(lastScan.started_at).toLocaleString('tr-TR',{hour:'2-digit',minute:'2-digit',day:'2-digit',month:'2-digit'}):'—';
  ...
  const renderProduct=(p:Product)=>{const hasDrop=Number(p.discount_percent||0)>0,previous=Number(p.original_price||0),current=Number(p.current_price||0),label=p.metadata?.interest||categoryMap[p.category_id]||p.category_id;return <a key={p.id} href={p.url} target='_blank' rel='noreferrer' className={styles.product}><div className={styles.productImageWrap}>{p.image_url?<img src={p.image_url} alt='' loading='lazy' className={styles.productImage}/>:<span className={styles.imageFallback}><Store size={18}/></span>}</div><div className={styles.productMain}><div className={styles.productBadges}><span className={styles.storeBadge}>{siteMap[p.site_id]||p.site_id}</span><span className={styles.interestBadge}>{label}</span>{p.brand&&<span className={styles.brandBadge}>{p.brand}</span>}</div><div className={styles.productTitle}>{p.title}</div><div className={styles.priceRow}><strong>{current?`${current.toLocaleString('tr-TR')} TL`:'Fiyat yok'}</strong>{hasDrop&&previous>current&&<span>{previous.toLocaleString('tr-TR')} TL</span>}</div></div><div className={`${styles.discount} ${hasDrop?styles.discountActive:''}`}>{hasDrop?`-%${Math.round(Number(p.discount_percent))}`:<ExternalLink size={16}/>}</div></a>};
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <section className={styles.compactStats}><div><span>Takip</span><strong>{sectionProducts.length}</strong></div><div><span>Düşüş</span><strong>{realDrops}</strong></div><div><span>Profil fırsatı</span><strong>{familyProducts.length}</strong></div><div><span>Son tarama</span><strong>{lastScan?new Date(lastScan.started_at).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'}):'—'}</strong></div></section>

    <section className={`${styles.card} ${styles.smartCard}`}><div className={styles.cardHead}><div><div className={styles.cardTitle}><Sparkles size={16}/> Size uygun fırsatlar</div><div className={styles.muted}>Yalnız doğrulanmış {sectionTitle[section]} profili · kategori karışımı yok</div></div><button className={styles.refreshButton} onClick={refreshNow} disabled={refreshing}><RefreshCw size={16} className={refreshing?styles.spin:''}/></button></div><div className={styles.smartChips}>{interests.map(x=><span key={x}>{x}</span>)}</div><div className={styles.productList}>{familyProducts.length===0?<div className={styles.empty}>Bu profil için doğrulanmış yeni fırsat henüz yok.</div>:familyProducts.slice(0,40).map(renderProduct)}</div></section>

    <section className={`${styles.card} ${styles.productsCard}`}><div className={styles.cardHead}><div><div className={styles.cardTitle}><Store size={16}/> Ana fiyat takibi</div><div className={styles.muted}>Kategori + profil doğrulamalı · {sortedProducts.length} sonuç</div></div><button className={styles.refreshButton} onClick={refreshNow} disabled={refreshing}><RefreshCw size={16} className={refreshing?styles.spin:''}/></button></div><div className={styles.filterBar}><label className={styles.searchBox}><Search size={15}/><input value={search} onChange={e=>{setSearch(e.target.value);setProductLimit(24)}} placeholder='Ürün ara'/></label><select className={styles.storeSelect} value={storeFilter} onChange={e=>{setStoreFilter(e.target.value);setProductLimit(24)}}><option value='all'>Tüm mağazalar</option>{availableSiteIds.map(id=><option key={id} value={id}>{siteMap[id]||id}</option>)}</select><select className={styles.storeSelect} value={categoryFilter} onChange={e=>{setCategoryFilter(e.target.value);setProductLimit(24)}}><option value='all'>Tüm kategoriler</option>{availableCategoryIds.map(id=><option key={id} value={id}>{categoryMap[id]||id}</option>)}</select><button className={`${styles.dropToggle} ${dropsOnly?styles.dropToggleActive:''}`} onClick={()=>{setDropsOnly(v=>!v);setProductLimit(24)}}><SlidersHorizontal size={14}/> {dropsOnly?'Ucuzlayanlar':'Tümü'}</button></div><div className={styles.productList}>{sortedProducts.length===0?<div className={styles.empty}>Bu filtrede ürün yok.</div>:visibleProducts.map(renderProduct)}</div>{productLimit<sortedProducts.length&&<button className={styles.moreButton} onClick={()=>setProductLimit(v=>Math.min(v+24,sortedProducts.length))}>24 ürün daha göster</button>}</section>
