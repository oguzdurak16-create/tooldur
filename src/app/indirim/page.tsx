...
import { Activity, Bell, ChevronDown, ExternalLink, Plus, RefreshCw, Search, Settings2, SlidersHorizontal, Store, Tag, Trash2 } from 'lucide-react';
...
export default function IndirimPage(){
  ...
  const sortedProducts=useMemo(()=>{
    ...
    return sectionProducts
      .filter(p=>storeFilter==='all'||p.site_id===storeFilter)
      .filter(p=>!dropsOnly||Number(p.discount_percent||0)>0)
      .filter(p=>!q||`${p.title} ${p.brand||''} ${siteMap[p.site_id]||p.site_id}`.toLocaleLowerCase('tr-TR').includes(q))
      .sort((a,b)=>{
  ...
  const refreshNow=async()=>{ setRefreshing(true); await load(true); setRefreshing(false); };
  ...
  const changeSection=(next:Section)=>{
    ...
    setProductLimit(24);setStoreFilter('all');setDropsOnly(false);setSearch('');
  ...
  const realDrops=sectionProducts.filter(p=>Number(p.discount_percent||0)>0).length;
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <section className={styles.compactStats}>
      <div><span>Ürün</span><strong>{sectionProducts.length}</strong></div><div><span>Düşüş</span><strong>{realDrops}</strong></div><div><span>Kural</span><strong>{activeRules}</strong></div><div><span>Son tarama</span><strong>{lastScan?new Date(lastScan.started_at).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'}):'—'}</strong></div>
    </section>

    <section className={`${styles.card} ${styles.productsCard}`}>
      <div className={styles.cardHead}><div><div className={styles.cardTitle}><Store size={16}/> Fiyat takibi</div><div className={styles.muted}>Gerçek düşüş yüksekten düşüğe · {sortedProducts.length} sonuç</div></div><button className={styles.refreshButton} onClick={refreshNow} disabled={refreshing} aria-label='Listeyi yenile'><RefreshCw size={16} className={refreshing?styles.spin:''}/></button></div>
      <div className={styles.filterBar}>
        <label className={styles.searchBox}><Search size={15}/><input value={search} onChange={e=>{setSearch(e.target.value);setProductLimit(24)}} placeholder='Ürün ara'/></label>
        <select className={styles.storeSelect} value={storeFilter} onChange={e=>{setStoreFilter(e.target.value);setProductLimit(24)}}><option value='all'>Tüm mağazalar</option>{availableSiteIds.map(id=><option key={id} value={id}>{siteMap[id]||id}</option>)}</select>
        <button className={`${styles.dropToggle} ${dropsOnly?styles.dropToggleActive:''}`} onClick={()=>{setDropsOnly(v=>!v);setProductLimit(24)}}><SlidersHorizontal size={14}/> {dropsOnly?'Ucuzlayanlar':'Tümü'}</button>
      </div>
      <div className={styles.productList}>{sortedProducts.length===0?<div className={styles.empty}>Bu filtrede ürün yok.</div>:visibleProducts.map(p=>{const hasDrop=Number(p.discount_percent||0)>0;const previous=Number(p.original_price||0);const current=Number(p.current_price||0);return <a key={p.id} href={p.url} target='_blank' rel='noreferrer' className={styles.product}>
        <div className={styles.productImageWrap}>{p.image_url?<img src={p.image_url} alt='' loading='lazy' className={styles.productImage} onError={e=>{e.currentTarget.style.visibility='hidden'}}/>:<span className={styles.imageFallback}><Store size={18}/></span>}</div>
        <div className={styles.productMain}><div className={styles.productBadges}><span className={styles.storeBadge}>{siteMap[p.site_id]||p.site_id}</span>{p.brand&&<span className={styles.brandBadge}>{p.brand}</span>}{p.in_stock===false&&<span className={styles.outBadge}>Stok yok</span>}</div><div className={styles.productTitle}>{p.title}</div><div className={styles.priceRow}><strong>{current?`${current.toLocaleString('tr-TR')} TL`:'Fiyat yok'}</strong>{hasDrop&&previous>current&&<span>{previous.toLocaleString('tr-TR')} TL</span>}</div></div>
        <div className={`${styles.discount} ${hasDrop?styles.discountActive:''}`}>{hasDrop?`-%${Math.round(Number(p.discount_percent))}`:<ExternalLink size={16}/>}</div>
      </a>})}</div>
    ...
    <div className={styles.toolsGrid}>
      <details className={styles.collapse}><summary><span><Settings2 size={16}/> Yönetim</span><span className={styles.summaryMeta}>{visibleRules.length} kural <ChevronDown size={16}/></span></summary><div className={styles.collapseBody}>
        <div className={styles.sectionLabel}><Plus size={15}/> Yeni takip kuralı</div><div className={styles.formGrid}>
          ...
          <label className={styles.field}><span className={styles.label}>Gerçek düşüş eşiği (%)</span><input className={styles.input} type='number' min={1} max={95} value={minDiscount} onChange={e=>setMinDiscount(Number(e.target.value)||1)}/></label><div className={styles.field}><button className={styles.button} onClick={addRule} disabled={saving}>{saving?'Ekleniyor…':'Takibe Ekle'}</button></div>
        </div>{error&&<div className={styles.error}>{error}</div>}{message&&<div className={styles.success}>{message}</div>}
        <div className={styles.sectionLabel}><Tag size={15}/> Takip kuralları</div><div className={styles.rules}>{loading?<div className={styles.empty}>Yükleniyor…</div>:visibleRules.length===0?<div className={styles.empty}>Bu sekmede takip kuralı yok.</div>:visibleRules.map(rule=><div className={styles.rule} key={rule.id}><div className={styles.ruleTop}><div><div className={styles.ruleName}>{categoryMap[rule.category_id]||rule.category_id} · {siteMap[rule.site_id]||rule.site_id}</div><div className={styles.ruleMeta}>%{Number(rule.min_discount_percent)} gerçek düşüş ve üzeri</div></div><button aria-label='Kuralı aç/kapat' className={`${styles.switch} ${rule.enabled?styles.switchOn:''}`} onClick={()=>toggleRule(rule)}><span className={styles.switchKnob}/></button></div><div className={styles.ruleUrl}>{rule.category_url||'Kategori URL yok'}</div><div className={styles.chips}><span className={`${styles.chip} ${rule.notify?styles.chipOn:''}`}>{rule.notify?'Bildirim açık':'Bildirim kapalı'}</span><button className={`${styles.ghost} ${styles.danger}`} onClick={()=>removeRule(rule.id)}><Trash2 size={12}/> Sil</button></div></div>)}</div>
      </div></details>

      <details className={styles.collapse}><summary><span><Bell size={16}/> Bildirim ayarları</span><span className={styles.summaryMeta}>{settings.notifications_enabled?'Açık':'Kapalı'} <ChevronDown size={16}/></span></summary><div className={styles.collapseBody}>
