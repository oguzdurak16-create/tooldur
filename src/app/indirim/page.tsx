...
const sectionCategoryIds:Record<Section,Set<string>>={
  merve:new Set(['kadin-giyim','kozmetik','ayakkabi','canta','ev-yasam','merve-bakim','merve-ev']),
  oguzhan:new Set(['erkek-giyim','oguzhan-elektronik','oguzhan-arac','oguzhan-bakim']),
  ege:new Set(['cocuk','ege-bakim','ege-oyuncak','ege-kitap']),
};
const belongs=(categoryId:string,section:Section)=>sectionCategoryIds[section].has(categoryId)||categoryId.startsWith(`${section}-`);
const smartSources:Record<Section,{key:string;interest:string;site:string;category:string}[]>={
  merve:[
    {key:'gratis-bakim',interest:'Kişisel bakım',site:'gratis',category:'kadin-giyim'},
    {key:'amazon-ev-bakim',interest:'Ev & temizlik',site:'amazon-tr',category:'kadin-giyim'},
    {key:'amazon-kadin-ayakkabi-canta',interest:'Ayakkabı & çanta',site:'amazon-tr',category:'kadin-giyim'},
  ],
  oguzhan:[
    {key:'amazon-elektronik',interest:'Elektronik',site:'amazon-tr',category:'erkek-giyim'},
    {key:'amazon-pc-telefon',interest:'PC & telefon',site:'amazon-tr',category:'erkek-giyim'},
    {key:'amazon-el-aletleri',interest:'El aletleri',site:'amazon-tr',category:'erkek-giyim'},
    {key:'amazon-arac',interest:'Megane 2 bakım & aksesuar',site:'amazon-tr',category:'erkek-giyim'},
  ...
  ege:[
    ...
    {key:'ebebek-oyuncak',interest:'Oyuncak & aktivite',site:'ebebek',category:'cocuk'},
    {key:'ebebek-beslenme',interest:'Suluk & beslenme',site:'ebebek',category:'cocuk'},
    {key:'amazon-ege-kitap',interest:'Kitap & etkinlik',site:'amazon-tr',category:'cocuk'},
  ],
};
...
export default function IndirimPage(){
  ...
  const visibleSmart=useMemo(()=>smartProducts.filter(p=>p.category_id===sectionDefaultCategory[section]).sort((a,b)=>Number(b.discount_percent||0)-Number(a.discount_percent||0)||Number(a.current_price||99999999)-Number(b.current_price||99999999)).slice(0,40),[smartProducts,section]);
  ...
  const load=async(silent=false)=>{
    ...
    const [c,s,r,p,st,l]=await Promise.all([
      ...
      supabase.from('merve_products').select('id,title,brand,image_url,site_id,category_id,current_price,original_price,discount_percent,in_stock,url').order('discount_percent',{ascending:false,nullsFirst:false}).order('current_price',{ascending:true,nullsFirst:false}).limit(700),
  ...
  const loadSmart=async()=>{
    ...
    const snapshotKey='tooldur-indirim-smart-price-v2';
    ...
    await Promise.all((Object.keys(smartSources) as Section[]).flatMap(sec=>smartSources[sec].map(async src=>{
      try{
        ...
        data.products.slice(0,40).forEach((raw:any,i:number)=>{
          const current=Number(raw.current_price||0);if(!current||!raw.url||raw.in_stock===false)return;
  ...
  const heroText=section==='oguzhan'?'XL giyim · elektronik · el aletleri · Megane 2 · bakım':section==='ege'?'4+ erkek çocuk · oyuncak · kitap · banyo · beslenme':'L/XL giyim · ayakkabı/çanta · kişisel bakım · ev ihtiyaçları';
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <section className={`${styles.card} ${styles.smartCard}`}><div className={styles.cardHead}><div><div className={styles.cardTitle}><Sparkles size={16}/> Size uygun fırsatlar</div><div className={styles.muted}>Profil odaklı · ilk fiyat referans, sonraki gerçek düşüşler yeşil görünür</div></div><button className={styles.refreshButton} onClick={refreshNow} disabled={refreshing||smartLoading}><RefreshCw size={16} className={(refreshing||smartLoading)?styles.spin:''}/></button></div><div className={styles.smartChips}>{smartSources[section].map(x=><span key={x.key}>{x.interest}</span>)}</div><div className={styles.productList}>{smartLoading&&visibleSmart.length===0?<div className={styles.empty}>Size uygun ürünler taranıyor…</div>:visibleSmart.length===0?<div className={styles.empty}>Bu kaynaklardan ürün alınamadı.</div>:visibleSmart.map(renderProduct)}</div></section>
